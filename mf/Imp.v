...
(**
    _Theorem_: For all arithmetic expressions [a],  aeval (optimize_0plus a) = aeval a.

    _Proof_: By induction on [a].  The [AMinus] and [AMult] cases
    follow directly from the IH.
  ...
  - (* APlus *)
    destruct a1 eqn:Ea1;
      ...
      try (simpl; simpl in IHa1; rewrite IHa1;
           rewrite IHa2; reflexivity).
    (* In the following case [try] does nothing, and we need to follow the script*)
    + (* a1 = ANum n *) destruct n eqn:En;
...
(** This is a loop in tactic execution, not in Coq!*)
...
Ltac inv_and_sub H := inversion H; subst; clear H; trivial.

(** This defines a new tactic called [inv_and_sub] that takes a hypothesis
...
Lemma inv_and_sub_example1: forall {a b c: nat}, [a ;b] = [a;c] -> b = c.
  intros.
  inv_and_sub H.
Qed.
...
(** Finally, here are some miscellaneous tactics that you may find
    convenient.
     ...
     - [constructor]: Try to find a constructor [c] (from some
       ...
       applied to solve the current goal.  If  found, behave
       like [apply c].
...
(** We have presented [aeval] and [beval] as functions defined by
    [Fixpoint]s.  Another way to think about evaluation is  as a _relation_ between
...
Proof.
  ...
  - intros H. induction H.
    + simpl. reflexivity.
    + simpl. reflexivity.
    + simpl. rewrite aeval_iff_aevalR in H, H0. subst. reflexivity.
    + simpl. rewrite aeval_iff_aevalR in H, H0. subst. reflexivity.
    + simpl. subst. reflexivity.
    + simpl. subst. reflexivity.
  - intros H. generalize dependent bv. induction b.
    + simpl. intros bv H'. subst. constructor.
    + simpl. intros bv H'. subst. constructor.
    + simpl. intros bv H'. subst. constructor.
    ...
    + simpl. intros bv H'. subst. constructor.
      * apply aeval_iff_aevalR. reflexivity.
      * apply aeval_iff_aevalR. reflexivity.
    + simpl. intros bv H'. subst. constructor. apply IHb. reflexivity.
    + simpl. intros bv H'. subst. constructor.
...
Restart.
  ...
  - intros H.
    induction H; simpl;
      try (reflexivity);
      try (rewrite aeval_iff_aevalR in H, H0; subst; reflexivity);
      try (subst; reflexivity).
  - intros H. generalize dependent bv.
...
(** Here's the proof of determinism, first for a simpler language: *)
...
(** The proof is by induction on the first derivation and inversion on the second: *)

Theorem ceval_deterministic_simple: forall c st st1,
     st ==[ c ]=> st1  ->  forall st2, st ==[ c ]=> st2 ->  st1 = st2.
Proof.
intros c st st1 E1.
  induction E1.
  (* Case "E_Skip".*)
  - intros st2 E2. inversion E2. subst. reflexivity.
    (* Case "E_Ass". *)
  - intros st2 E2. inv_and_sub E2.
    (* Case "E_Seq".*)
  - intros st2 E2. inv_and_sub E2.
    apply IHE1_1 in H1. subst. apply IHE1_2 in H4. assumption.

Restart. (* some more automation*)
intros c st st1  E1.
induction E1;
           intros st2 E2; inv_and_sub E2.
(* base cases solved*)
...
(* ================================================================= *)
(** ** Evaluation is total (w/o loops) *)

(**  See final exercise for a more general case *)

Theorem ceval_total: forall c st, exists st', st ==[ c ]=> st'  .
Proof.
intros c.
induction c; intros.
(*Case "SKIP".*)
...
(* Case "Ass". *)
 - exists  (s !-> aeval  st a ; st). constructor. reflexivity.
(* Case "Seq".*)
 - destruct (IHc1 st) as [st' H']. (* <--  note instantiation with st*)
   destruct (IHc2 st') as [st'' H''].
   exists st''. apply E_Seq with st'; assumption.
Qed.

(** The above was quite a delicate proof:

- we must _not_ introduce [st]

- the elimination of [exists] is done after the right elimination of [forall]

- we must choose the right state for [exists] introduction.

The two former points: live with that. For the latter, automation can help.
*)

(* ----------------------------------------------------------------- *)
(** *** The [(e)auto] tactics and the  [Hint] declaration *)

(** [auto] implements a Prolog-like resolution procedure. It first
tries to solve the current goal using [assumption], then it reduces the goal
to an atomic one using [intros] and introducing the newly generated
hypotheses as hints. Then it looks at the list of tactics associated
to the head symbol of the goal and tries to [apply] one of them. This
process is recursively applied to the generated subgoals.

[eauto] does the same modulo unification rather than pattern-matching.
*)

(** By default, auto only uses the hypotheses of the current goal and
the hints of the database named [core].  With the following
declaration, we tell Coq to automatically use the constructors of an
inductive definition as introduction rules in tactics *)

Hint Constructors ceval : core.

(** Now the proof of totality requires less interaction
*)

Theorem ceval_total_h:  forall c st, exists st', st ==[ c ]=> st'  .
Proof.
intros c.
induction c; intros; eauto.
- eexists. constructor. reflexivity.
(* Case "Seq".*)
- destruct (IHc1 st) as [st' H'].
  destruct (IHc2 st').
  eauto.
Qed.
...
(** Back to the theorem for the whole language:*)
Theorem ceval_deterministic: forall c st st1,
     st =[ c ]=> st1  -> forall st2, st =[ c ]=> st2 ->
     st1 = st2.
Proof.
  intros c st st1 E1.
...
Inductive no_whilesR: com -> Prop :=
  ...
  | NW_Asgn :
    ...
    no_whilesR <{ x := a }>
  | NW_Seq :
    forall c1 c2 (H1 : no_whilesR c1) (H2 : no_whilesR c2),
    no_whilesR <{ c1; c2 }>
  | NW_If :
    forall b c1 c2 (H1 : no_whilesR c1) (H2 : no_whilesR c2),
...
Proof.
  ...
  - intros H. induction c;
      try (constructor);
      try (inversion H as [H']; apply andb_true_iff in H');
      try (destruct H' as [H1  H2]; apply IHc1; assumption);
      try (destruct H' as [H1  H2]; apply IHc2; assumption).
  - intros H. induction c; simpl;
      try (reflexivity);
      try (inversion H; subst; apply andb_true_iff; split);
...
Proof.
  intros c H st. generalize dependent st. induction c; intros.
  - exists st. apply E_Skip.
  - exists (x !-> (aeval st a) ; st). apply E_Asgn. reflexivity.
  - inversion H as [| | c1' c2' H1 H2 |]. subst. clear H.
    assert (H: exists st1, st =[ c1 ]=> st1).
      { apply IHc1. assumption. }
    clear H1. destruct H as [st1 H1].
    assert (H: exists st2, st1 =[ c2 ]=> st2).
      { apply IHc2. assumption. }
    clear H2. destruct H as [st2 H2].
    exists st2. apply E_Seq with (st' := st1); assumption.
  - inversion H as [| | | b' c1' c2' H1 H2]. subst. clear H.
    assert (H: exists st1, st =[ c1 ]=> st1).
      { apply IHc1. assumption. }
    clear H1. destruct H as [st1 H1].
    assert (H: exists st2, st =[ c2 ]=> st2).
      { apply IHc2. assumption. }
    clear H2. destruct H as [st2 H2].
